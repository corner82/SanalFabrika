...
ajaxACLResources.ajaxCallWidget('call');
    ...
    window.gridDetailClick= function (target) {
        ...
        $.ajax({
            url: 'https://proxy.sanalfabrika.com/SlimProxyBoot.php',
            data: { url:'pkGetConsUserConfirmationProcessDetails_sysOsbConsultants' ,
                    pk : $("#pk").val(),
                    profile_id : row.id,
                    language_code : $('#langCode').val()
                    }, 
            ...
            success: function (data, textStatus, jqXHR) {
                if(data.length!==0) {
                    ...
                    $("#name_surname").val(data[0]['name']+' '+data[0]['surname']);
                    ...
                    console.log(row);
                    $('#dropdownRoles').ddslick('selectByValue', 
                                                {index: ''+row.role_id+'' ,
                                                 text : ''+row.role_eng+''}
                                                );
    ...
    $('#grid_confirm_registration').datagrid({
            ...
            url : 'https://proxy.sanalfabrika.com/SlimProxyBoot.php',
            queryParams : { url:'pkGetConsPendingUser_sysOsbConsultants' ,
                            pk : $('#pk').val(),
                            language_code : $('#langCode').val()}, 
            ...
            columns:
                [[
                    {field:'id',title:'ID'},
                    {field:'username',title:'Kullanıcı Hesabı',sortable:true,width:250},
                    //{field:'operation_name',title:'İşlem',sortable:true, width:100},
                    {field:'name_surname',title:'Kullanıcı Adı',sortable:true, width:250},
                    ...
                    {field:'detay',title:'Detay',sortable:false, align:'center', 
                        ...
                        formatter:function(value,row,index){
                            ...
                            var e = '<button style="padding : 2px 4px;" title="Pasif yap"  class="btn btn-primary" type="button" onclick="return gridDetailClick(this);"><i class="fa fa-plus"></i></button>';
                            return e;
                        }
                    },
...
window.updateApprovalWrapper = function (e) {
 e.preventDefault();
  ...
  var ddOperationsData = $('#dropdownRoles').data('ddslick');
 if(!ddOperationsData.selectedData.value > 0) {
    wm.warningMessage('resetOnShown');
    wm.warningMessage('show', 'Kullanıcı Rolü Seçiniz', 'Lütfen kullanıcı rolü seçiniz!');
    return false;
}
